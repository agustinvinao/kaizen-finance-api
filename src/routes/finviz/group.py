...
@finviz.route(
    f"/{api_path}/{module_path}/{submodule_path}/screener_view/overview/<sector>",
    methods=["GET"],
)
def screener_view_overview(sector=None):
    if request.method == "GET":
        fgoverview = Overview()
        df = (
            fgoverview.screener_view()
            if sector == None
            else fgoverview.screener_view(sector)
        )
...
@finviz.route(
    f"/{api_path}/{module_path}/{submodule_path}/screener_view/performance/<sector>",
    methods=["GET"],
)
def screener_view_performance(sector=None):
    if request.method == "GET":
        ...
        df = (
            fgperformance.screener_view()
            if sector == None
            else fgperformance.screener_view(sector)
        )
        return jsonify(df.to_dict(orient="records"))
