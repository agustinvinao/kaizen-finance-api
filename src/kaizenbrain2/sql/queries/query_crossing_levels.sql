...
-- WEEKLY
SELECT 	to_char(now() - lt.dt, 'dd "days" ago') AS ago,
		lt.symbol,
		t.dt::date AS dt,
		la.level / 100 as level,
		CASE
...
SELECT 	to_char(now() - lt.dt, 'dd "days" ago') AS ago,
        ...
        END AS crossing,
        ll.dist,
        lt.close / ll.price - 1 AS movement
FROM last_ticks_1w lt
	CROSS JOIN LATERAL (SELECT *, CASE WHEN t.open > t.close THEN 'desc' WHEN t.open < t.close THEN 'asc' END AS direction
						FROM ticks_1w t WHERE lt.symbol = t.symbol ORDER BY dt DESC LIMIT 1) t
	CROSS JOIN LATERAL (SELECT ll.price, ll.level, abs(t.close - ll.price) / t.close AS dist
    					FROM levels_longterm ll WHERE lt.symbol = ll.symbol ORDER BY dist LIMIT 1) ll
