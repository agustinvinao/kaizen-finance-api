CREATE MATERIALIZED VIEW phases_crypto_ticks_4h AS
WITH pivots AS (
	SELECT 	t.symbol, dt AS dt, high, low, open, close, volume,
			CASE 
		    	WHEN 	MAX(high) 	OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 5 PRECEDING AND 5 FOLLOWING) = high THEN 'DOWN'
		    	WHEN 	MIN(low) 	OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 5 PRECEDING AND 5 FOLLOWING) = low THEN 'UP'
		    ELSE NULL END AS pivot,
		    CASE
		    	WHEN
		    		(CASE WHEN max(high) OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 1 PRECEDING AND 10 FOLLOWING) = high THEN high ELSE NULL END) IS NOT NULL
		    		AND (CASE WHEN min(low) OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 1 PRECEDING AND 10 FOLLOWING) = low THEN low ELSE NULL END) IS NULL
		    	THEN 1
		    	WHEN
		    		(CASE WHEN min(low) OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 1 PRECEDING AND 10 FOLLOWING) = low THEN low ELSE NULL END) IS NOT NULL
		    		AND (CASE WHEN max(high) OVER(PARTITION BY t.symbol ORDER BY t.symbol, dt DESC ROWS BETWEEN 1 PRECEDING AND 10 FOLLOWING) = high THEN high ELSE NULL END) IS NULL
		    	THEN -1
		    END as dir_flag
	FROM assets a
		JOIN ticks_4h t ON a.symbol = t.symbol
	WHERE a.category = 'crypto'::asset_category
)
, pivots_with_dir AS (
...
CREATE INDEX phases_crypto_ticks_4h_symbol_phase_number ON phases_crypto_ticks_4h (symbol, phase_number);

		
REFRESH MATERIALIZED VIEW phases_crypto_ticks_4h;
DROP MATERIALIZED VIEW phases_crypto_ticks_4h;