CREATE TABLE IF NOT EXISTS ticks_1d (
...
CREATE INDEX ON ticks_1d (symbol, dt DESC);
-- SELECT create_hypertable('ticks', by_range('dt'), if_not_exists => TRUE);
SELECT create_hypertable('ticks_1d', 'dt');

-- enable compression
ALTER TABLE ticks_1d SET (timescaledb.compress,
