CREATE TABLE IF NOT EXISTS ticks_1h (
...
CREATE INDEX ON ticks_1h (symbol, dt DESC);
-- SELECT create_hypertable('ticks', by_range('dt'), if_not_exists => TRUE);
SELECT create_hypertable('ticks_1h', 'dt');

-- enable compression
ALTER TABLE ticks_1h SET (timescaledb.compress,
